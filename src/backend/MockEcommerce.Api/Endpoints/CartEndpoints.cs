...
public static class CartEndpoints
{
    ...
    public static void MapCartEndpoints(this WebApplication app)
    {
        ...
        group.MapPut("/{productId:int}", UpdateCartItem)
            ...
            .WithSummary("Sets the quantity of an existing cart item to an absolute value.");
        ...
        ICartService cartService)
    {
        ...
        if (currentQty + request.Quantity > 5)
        {
            return TypedResults.ValidationProblem(new Dictionary<string, string[]>
            {
                { "quantity", [$"Cannot add {request.Quantity} unit(s). Maximum quantity per product is 5. You already have {currentQty} in your cart."] }
            });
        }
        ...
        return existing is null
            ? TypedResults.Created("/api/cart", result)
            : TypedResults.Ok(result);
    }

    /// <summary>Sets the quantity of an existing cart item to an absolute value.</summary>
        ...
        ICartService cartService)
    {
        ...
        var result = cartService.Add(updatedItem);
